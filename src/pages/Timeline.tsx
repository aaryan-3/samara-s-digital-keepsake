      ...
      <div className="max-w-md mx-auto relative mb-12">
        ...
        <div className="space-y-8">
          {events.map((event, i) => (
            <div
              key={i}
              className={`fade-in-up stagger-${i + 1} soft-card relative ${event.highlight ? "border-primary/30 bg-warm-pink/20 shadow-glow" : ""}`}
            >
              {/* Dot on timeline */}
              <div className={`absolute left-1/2 -top-3 w-6 h-6 rounded-full border-2 -translate-x-1/2 ${event.highlight ? "bg-primary border-primary" : "bg-background border-primary/40"}`} />
              ...
              {event.hasPhoto && (
                <div className="mt-4 aspect-square max-w-[200px] mx-auto rounded-xl bg-primary/10 flex items-center justify-center text-3xl border-2 border-dashed border-primary/30">
                  📷
                  <p className="text-xs text-muted-foreground ml-2">Add photo</p>
                </div>
              )}
            </div>
          ))}
